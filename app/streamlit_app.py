...
def pisos_to_df(bd: BuildingDefinition) -> pd.DataFrame:
    rows = [{"Piso": p.index, "Título": p.title, "Planta": p.plan_tag, "Cota (m)": p.elevation,
...
def df_to_pisos(df: pd.DataFrame, bd: BuildingDefinition) -> tuple[PisoDefinition, ...]:
    pisos = []
    df = df.dropna(subset=["Planta"]).copy()
    df = df.sort_values("Cota (m)").reset_index(drop=True)
    for i, row in df.iterrows():
        idx = int(row["Piso"]) if pd.notna(row["Piso"]) else i + 1
        mats = {"pilares": str(row["fck pilares"] or "C40").upper(), "vigas": str(row["fck vigas"] or "C40").upper(),
                "lajes": str(row["fck lajes"] or "C40").upper()}
        pisos.append(PisoDefinition(idx, str(row["Título"] or bd.plans[row["Planta"]].name), float(row["Cota (m)"]),
                                    float(row["Pé-direito (m)"]), str(row["Planta"]), mats))
    # reindexar de baixo para cima (o ETABS exige stories em ordem)
    return tuple(replace(p, index=i + 1) for i, p in enumerate(pisos))
...
def build_config(base: Config, ui: dict) -> Config:
    return Config(
        ...
        etabs=replace(base.etabs, default_material=ui["default_material"], decimal_separator=ui["decimal"],
                      export_loads=ui["loads"], split_walls_at_nodes=True),
        source=base.source)
...
with st.sidebar:
    ...
    ui = {
        ...
        "loads": st.toggle("Exportar cargas de uso (ADI/DIS)", base_config.etabs.export_loads),
...
tab_pisos, tab_plantas, tab_cat, tab_diag = st.tabs(["Pavimentos", "Plantas", "Concreto", "Diagnóstico da varredura"])
...
with tab_pisos:
    ...
    if colC.button("➕ Adicionar pavimento no topo", use_container_width=True):
        ...
        df.loc[len(df)] = {"Piso": len(df) + 1, "Título": proto.title if proto else bd.plans[add_plan].name,
...
if gen:
    try:
        pisos = df_to_pisos(st.session_state.get("pisos_df_edited", st.session_state["pisos_df"]), bd)
        edited_bd = replace(bd, pisos=pisos, base_elevation=pisos[0].elevation - pisos[0].height if pisos else bd.base_elevation)
